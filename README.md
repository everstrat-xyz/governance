...
_As of 2026-09-15 12:40 UTC. Read live from `Timelock.getOperationState(opId)`
(`0` Unset · `1` Waiting · `2` Ready · `3` Done) and the Safe Transaction Service._

| Proposal | Where it stands |
|---|---|
| 001–007 | **Done** — executed on mainnet, state re-verified on-chain |
| **008** | **Ready** — executable by anyone since **2026-09-15 11:05:47 UTC**. Not executed yet (all three ops read `Ready`). |
| **009** | **Scheduled** — executable from **2026-09-15 12:38:35 UTC**. |
...
The DAO Safe holds **no pending transactions** (next nonce 22). Once an operation is
`Ready`, `execute` is permissionless — anyone can run it and only pays gas.
...
| [008](008-oracle-staleness-margin/) | Oracle: widen WETH/`address(0)` staleness 3600→4200 and USDC 82800→84600 (jitter margin, ×3) | **Ready** — scheduled 2026-09-13 11:05:47 UTC (tx `0x4be3e8c7…2afd4180`); executable by anyone since **2026-09-15 11:05:47 UTC**, not yet executed (3/3 ops state `Ready`) | `0xc6360fe9…8abaada9` (WETH), `0x209d4394…5492ad272166` (`address(0)`), `0x74253eb4…8840d93fe` (USDC) |
| [009](009-strategy-manager-add-unicl-strategies/) | StrategyManager: register the two UniCL USDC/WETH strategies (`addStrategy` ×2, batch) | **Scheduled** — scheduled 2026-09-13 12:38:35 UTC (tx `0xf383aaaf…7d88a59`); executable from **2026-09-15 12:38:35 UTC** | `0x345dfba9…5e7eca8` (0.3% pool), `0xf340bad1…828ea50990` (0.01% pool) |
